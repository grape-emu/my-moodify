// Begin at convertGoogleCloudVisionObjToSpotifyString (line 116)
...
class SpotifyUrlObject {
  ...
  constructor(name,joyScale,sorrowScale,surpriseScale,angerScale,radius = 0.4,minVal = 0,maxVal = 1) {
    ...
    // In this.printString, we use a fixed point radius to give spotify two values (min and max) for each tag; this tells the function how big that range should be
    this.radius = radius;
    // Most tags are ranged from 0-1, but some have other values, thus the defaults (for these and also this.radius)
    this.minVal = minVal;
    this.maxVal = maxVal;
  }
